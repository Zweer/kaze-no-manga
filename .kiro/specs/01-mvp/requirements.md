...
Unlike Mihon (which is headless/local-only), we have a server that stores images on S3
...
from S3), and track progress across devices.
...
Search (live on external sources)
  ...
  → User opens a chapter → images downloaded to S3 on-demand (sync, user waits)
  → Reader: vertical scroll from S3, chapter transitions
...
### 3. Add to DB
- When user selects a manga from search results, save to DynamoDB:
  - Manga metadata (title, cover, source, source ID)
  - Chapter list (number, title, source URL)
...
- **Lazy download**: when user opens a chapter not yet on S3, download it
  synchronously (user sees loading state, then reads)
- Infinite chapter transition (finish chapter → next loads automatically)
- If chapter is already on S3 (another user triggered download), serve directly
...
- **Sync download** — user waits for chapter images to be on S3 before reading
...
- User can read a chapter (images from S3)
